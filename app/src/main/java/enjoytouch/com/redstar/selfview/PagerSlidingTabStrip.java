...
public class PagerSlidingTabStrip extends LinearLayout {
    ...
    //private int tabSpace = 15;//tab???????????
    private Locale locale;
    private int indicatorHeight = 2;
    private int indicatorColor =getResources().getColor(R.color.xiahuaxian) ;//??????????

    private int indicatorExtraWidth=0;// ???????


    private int tabTextSize = 15;//textview????????????????dp
    private int tabPadding = 21;//textview??padding(????????? expand???????true??????? ????dp
    private int tabTextColor = getResources().getColor(R.color.text1);//????????
    private int tabSelectedColor = getResources().getColor(R.color.text1);//?????????font03????
    private int bottomSpace = 9;//???????????????view????????????
    private int tabScrollColor = this.getContext().getResources().getColor(R.color.gray);//???????textview??????
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        tabsContainer.setLayoutParams(new LayoutParams(shouldExpand ? LayoutParams.MATCH_PARENT : LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT));
        ...
        //tabSpace = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, tabSpace, dm);
    ...
    public void setLoginCheck(String num){
        this.loginCheck = num;
    }
    ...
    public void notifyDataSetChanged() {
        ...
        ContentUtil.makeLog("tab_s", String.valueOf(pager.getCurrentItem()));

        if (v instanceof TextView) {
            ((TextView) v).setTextColor(tabSelectedColor);
        }
    ...
    private void addTextTab(final int position, String title) {
        ...
        tab.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (loginCheck.contains(position + "")) {
                    if (AccountUtil.showLoginView(getContext())) return;
                }
    ...
    private void updateTabStyles() {
        /*defaultTabLayoutParams.rightMargin = tabSpace;
		expandedTabLayoutParams.rightMargin = tabSpace;*/

        for (int i = 0; i < tabCount; i++) {

            View v = tabsContainer.getChildAt(i);
            if (i == tabCount - 1) {
                defaultTabLayoutParams.rightMargin = 0;
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (!checkedTabWidths && childWidth > 0 && myWidth > 0) {
            ...
            if (childWidth <= myWidth) {
                for (int i = 0; i < tabCount; i++) {
                    if (i == tabCount - 1) {
                        expandedTabLayoutParams.rightMargin = 0;
                    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        final int height = getHeight() - DisplayUtil.dip2px(getContext(), bottomSpace);
        ...
        canvas.drawRect(lineLeft-DisplayUtil.dip2px(getContext(),20), height - indicatorHeight, lineRight+DisplayUtil.dip2px(getContext(),20), height, rectPaint);
    ...
    public void setindicatorExtraWidth(int indicatorExtraWidth) {
        this.indicatorExtraWidth = indicatorExtraWidth;
        invalidate();
    }
    ...
    public void setTabsContainer(LinearLayout tabsContainer) {
