...
//        if(isLoadding)dialog.show();
        Call<CollectionBean> call= HttpServiceClient.getInstance().user_product_collection(MyApplication.token,"2" ,MyApplication.cityId,PAGE,SIZE);
...
//                if (isLoadding) dialog.dismiss();
    ...
    private void mergeData(List<MyCollectionBean>dataBeans){
        if(dataBeans!=null){
